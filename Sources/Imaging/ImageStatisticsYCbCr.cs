...
namespace AForge.Imaging
{
    ...
    public class ImageStatisticsYCbCr
    {
        ...
        /// 
        public ImageStatisticsYCbCr( AForge.Bitmap image )
        {
            CheckSourceFormat( image.PixelFormat );

            // lock AForge.Bitmap data
            AForge.BitmapData imageData = image.LockBits(
                new Rectangle( 0, 0, image.Width, image.Height ),
                AForge.ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb );
        ...
        /// <exception cref="ArgumentException">Mask must have the same size as the source image to get statistics for.</exception>
        /// 
        public ImageStatisticsYCbCr( AForge.Bitmap image, byte[,] mask )
        {
            CheckSourceFormat( image.PixelFormat );
