...
namespace AForge.Imaging.ColorReduction
{
    ...
    /// AForge.Bitmap newImage = ciq.ReduceColors( image, 16 );
    /// </code>
    /// 
    ...
    public class ColorImageQuantizer
    {
        ...
        /// </summary>
        /// 
        /// <remarks><para>The property has effect only for methods like <see cref="ReduceColors(AForge.Bitmap, int)"/> and
        ...
        /// 
        public Color[] CalculatePalette( AForge.Bitmap image, int paletteSize )
        {
            AForge.BitmapData data = image.LockBits( new Rectangle( 0, 0, image.Width, image.Height ),
        ...
        /// 
        public AForge.Bitmap ReduceColors( AForge.Bitmap image, int paletteSize )
        {
            AForge.BitmapData data = image.LockBits( new Rectangle( 0, 0, image.Width, image.Height ),
                AForge.ImageLockMode.ReadOnly, image.PixelFormat );
            ...
            try
            {
                AForge.Bitmap result = ReduceColors( new UnmanagedImage( data ), paletteSize );
        ...
        /// 
        public AForge.Bitmap ReduceColors( UnmanagedImage image, int paletteSize )
        {
        ...
        /// 
        public AForge.Bitmap ReduceColors( AForge.Bitmap image, Color[] palette )
        {
            AForge.BitmapData data = image.LockBits( new Rectangle( 0, 0, image.Width, image.Height ),
                AForge.ImageLockMode.ReadOnly, image.PixelFormat );
            ...
            try
            {
                AForge.Bitmap result = ReduceColors( new UnmanagedImage( data ), palette );
                if ( ( image.HorizontalResolution > 0 ) && ( image.VerticalResolution > 0 ) )
                {
                    result.SetResolution( image.HorizontalResolution, image.VerticalResolution );
                }
        ...
        /// <exception cref="ArgumentException">Invalid size of the target color palette.</exception>
        /// 
        public AForge.Bitmap ReduceColors( UnmanagedImage image, Color[] palette )
        {
            if ( ( image.PixelFormat != PixelFormat.Format24bppRgb ) &&
                 ( image.PixelFormat != PixelFormat.Format32bppRgb ) &&
                 ( image.PixelFormat != PixelFormat.Format32bppArgb ) &&
            ...
            AForge.Bitmap destImage = new AForge.Bitmap( width, height, ( palette.Length > 16 ) ?
                PixelFormat.Format8bppIndexed : PixelFormat.Format4bppIndexed );
            // and init its palette
            AForge.Bitmap.ColorPalette cp = destImage.Palette;
            ...
            AForge.BitmapData destData = destImage.LockBits( new Rectangle( 0, 0, width, height ),
                AForge.ImageLockMode.ReadWrite, destImage.PixelFormat );
