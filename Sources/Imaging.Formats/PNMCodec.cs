...
namespace AForge.Imaging.Formats
{
    ...
    public class PNMCodec : IImageDecoder
    {
        ...
        /// 
        public AForge.Bitmap DecodeSingleFrame( Stream stream )
        {
        ...
        /// <exception cref="ArgumentException">The stream contains invalid (broken) PNM image.</exception>
        /// 
        public AForge.Bitmap DecodeFrame( int frameIndex, out ImageInfo imageInfo )
        {
            ...
            // read required frame
            AForge.Bitmap image = ReadImageFrame( stream, this.imageInfo );
        ...
        private AForge.Bitmap ReadImageFrame( Stream stream, PNMImageInfo imageInfo )
        {
            try
            {
        ...
        private unsafe AForge.Bitmap ReadP5Image( Stream stream, int width, int height, int maxValue )
        {
            double scalingFactor = (double) 255 / maxValue;

            // create new AForge.Bitmap and lock it
            AForge.Bitmap image = Tools.CreateGrayscaleImage( width, height );
            AForge.BitmapData imageData = image.LockBits( new Rectangle( 0, 0, width, height ),
        ...
        private unsafe AForge.Bitmap ReadP6Image( Stream stream, int width, int height, int maxValue )
        {
            double scalingFactor = (double) 255 / maxValue;

            // create new AForge.Bitmap and lock it
            AForge.Bitmap image = new AForge.Bitmap( width, height, PixelFormat.Format24bppRgb );
            AForge.BitmapData imageData = image.LockBits( new Rectangle( 0, 0, width, height ),
                AForge.ImageLockMode.ReadWrite, PixelFormat.Format24bppRgb );
