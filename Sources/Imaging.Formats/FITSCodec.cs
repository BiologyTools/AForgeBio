...
namespace AForge.Imaging.Formats
{
    ...
    public class FITSCodec : IImageDecoder
    {
        ...
        private unsafe AForge.Bitmap ReadImageFrame( Stream stream, FITSImageInfo imageInfo )
        {
            int width  = imageInfo.Width;
            int height = imageInfo.Height;

            // create new AForge.Bitmap
            AForge.Bitmap image = ( imageInfo.BitsPerPixel == 8 ) ?
                Tools.CreateGrayscaleImage( width, height ) :
                new AForge.Bitmap( width, height, PixelFormat.Format16bppGrayScale );
